package com.salezrobot.utilities;
...
public abstract class Reporter extends AbstractTestNGCucumberTests {
	...
	public void startReport() {
		...
		spark.config().setDocumentTitle("rsoft TEST REPORT");
		spark.config().setReportName("rsoft AUTOMATION DASHBOARD"); 
    ...
    public void reportStep2(String dec, String status, boolean bSnap ) {
    	Media img = null;
		if(bSnap && !status.equalsIgnoreCase("INFO")){
			long snapNumber = 100000L;
			snapNumber = takeSnap();
    ...
    public void reportStep2(String desc, String status) {
		reportStep2(desc, status, true);
	}
