...
import com.salezrobot.rsoft_web_Application_All_Elements.Rsoft_web_Application_All_Elements_and_All_Methods;
import com.salezrobot.salezrobot_Website_Clickable_Elements.Salezrobot_Website_Chatbot_Element;
import com.salezrobot.salezrobot_Website_Clickable_Elements.Salezrobot_Website_Clickable_Elements;
...
public class SalezRobot_Website extends Salezrobot_Website_Clickable_Elements{
	...
	public void appScrollingFunction() throws InterruptedException{
		Thread.sleep(8000);
		try {
			 WebElement close = driver.findElement(By.xpath("//button[@id='closeClick']"));
	    	WebDriverWait a = new WebDriverWait(driver, Duration.ofSeconds(30));
	    	a.until(ExpectedConditions.elementToBeClickable(close));
	    	close.click();
...
//        }
	...
	public WebElement facebook(int num) throws InterruptedException {
		...
		fbLinkButton(num).click();
			...
			System.out.println("Stale ELement FB");
		}
        Thread.sleep(3000);
	...
	public void twitter(int num) throws InterruptedException {
		scroll(twitterLinkButton(num));
		mouseMove(twitterLinkButton(num));
//		jsClick_Only(twitterLinkButton(num));
        ...
        try {
			WebElement twit = driver.findElement(By.xpath("(//span[text()='Rsoft Technologies Private Limited'])[1]"));
			if(twit.getText().equals("Rsoft Technologies Private Limited")) {
				...
				String a = "SalezRobot Twitter Page Text Changed<br >"+url();
				reportStep2(a, "FAIL");
			}
		}catch(Exception g) {
	...
	//Linkedin Function
	public void linkedin(int num) throws InterruptedException {
        ...
        try {
			WebElement linkedInPage = driver.findElement(By.xpath("//h1[text()[normalize-space()='SalezRobot CRM']]"));
	...
	public void youtube(int num) {
		 ...
		 ArrayList<String> arr3 = windowHandles();
	        ...
	        if(arr3.size()>1) {
	        	driver.switchTo().window(arr3.get(1));
	        try {
				WebElement youtube = driver.findElement(By.xpath("//yt-formatted-string[text()='RSoft Technologies']"));
				if(youtube.getText().equals("RSoft Technologies")) {
					String a = "SalezRobot Youtube Page is Dispalyed<br >"+url();
					reportStep2(a, "PASS");
					driver.close();
					Thread.sleep(1000);
					driver.switchTo().window(arr3.get(0));
				}else {
					String a = "SalezRobot Youtube Page Text Changed<br >"+url();
					reportStep2(a, "FAIL");
				}
			}catch(Exception g) {
				String a = "SalezRobot Youtube Page is Not Dispalyed<br >"+url();
				reportStep2(a, "FAIL");
				back();
			}
	        }else {
